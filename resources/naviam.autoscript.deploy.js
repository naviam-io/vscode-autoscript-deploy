    ...
    if (scriptConfig) {
        ...
        try {
            ...
            if (autoScriptSet.isEmpty()) {
                ...
                if (typeof scriptConfig._delete !== 'undefined' && scriptConfig._delete === true) {
                    autoscript.delete();
                    autoScriptSet.save();
                        ...
                        run: function () {
                            ...
                            try {
                                ...
                                if (bulletinBoard) {
                                    ...
                                    if (
                                        typeof error.getErrorGroup !== 'undefined' &&
                                        error.getErrorGroup() == 'script' &&
                                        error.getErrorKey() == 'errorrunningscript'
                                    ) {
                                        progress.error =
                                ...
                                if (
                                    ...
                                    var deployAutoScriptSet;
                                    try {
                                        deployAutoScriptSet = MXServer.getMXServer().getMboSet('AUTOSCRIPT', MXServer.getMXServer().getSystemUserInfo());
                                        var deploySqlf = new SqlFormat('autoscript = :1');
                                        deploySqlf.setObject(1, 'AUTOSCRIPT', 'AUTOSCRIPT', deployScript.getName());
    ...
    if (scriptSource) {
        if (languageHint === 'python' || languageHint === 'jython') {
            ...
            if (ast.type === 'Program' && ast.body) {
                ...
                var tryExtractConfigFromAssignedObjectLiteral = function (source) {
                    var assignmentRegex =
                        /(?:^|[^\w$])(?:scriptConfig|[A-Za-z_$][\w$]*\s*\.\s*scriptConfig|[A-Za-z_$][\w$]*\s*\[\s*['"]scriptConfig['"]\s*\])\s*=\s*/g;

                    var readBalancedObject = function (value, startIndex) {
                        var length = value.length;
                        var index = startIndex;

                        while (index < length && /\s/.test(value.charAt(index))) {
                            index++;
                        }
                        ...
                        for (; index < length; index++) {
                            ...
                            if (ch === '{') {
                                ...
                                if (depth === 0) {
                                    return value.substring(objectStart, index + 1);
                                }
                            }
                        }
                    ...
                    while (assignmentRegex.exec(source) !== null) {
                        var objectLiteral = readBalancedObject(source, assignmentRegex.lastIndex);
                        if (!objectLiteral) {
                            continue;
                        }

                        try {
                            var objectAst = parse('var __scriptConfigFallback = ' + objectLiteral + ';');
                            if (objectAst && objectAst.type === 'Program' && objectAst.body && objectAst.body.length > 0) {
                                var declaration = objectAst.body[0];
                                if (
                                    ...
                                    return astToJavaScript(declaration.declarations[0].init);
                                }
                            }
                        } catch (ignore) {
                            // Continue scanning other assignments if this one cannot be parsed.
                        }
                    }
                ...
                var isScriptConfigMemberExpression = function (node) {
                    if (!node || node.type !== 'MemberExpression') {
                        return false;
                    }

                    if (!node.computed && node.property && node.property.type === 'Identifier' && node.property.name === 'scriptConfig') {
                        return true;
                    }

                    if (node.computed && node.property && node.property.type === 'Literal' && node.property.value === 'scriptConfig') {
                        return true;
                    }
                ...
                var getScriptConfigValueNode = function (node) {
                    if (!node) {
                        return null;
                    }

                    if (node.type === 'VariableDeclarator') {
                        if (node.id && node.id.type === 'Identifier' && node.id.name === 'scriptConfig') {
                            return node.init || null;
                        }
                    }

                    if (node.type === 'AssignmentExpression') {
                        if (node.left && node.left.type === 'Identifier' && node.left.name === 'scriptConfig') {
                            return node.right || null;
                        }

                        if (isScriptConfigMemberExpression(node.left)) {
                            return node.right || null;
                        }
                    }
                ...
                var findScriptConfigValueNode = function (node) {
                    if (!node || typeof node !== 'object') {
                        return null;
                    }

                    var directMatch = getScriptConfigValueNode(node);
                    if (directMatch) {
                        return directMatch;
                    }
                    ...
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        if (Array.isArray(child)) {
                            for (var j = 0; j < child.length; j++) {
                                var nestedArrayResult = findScriptConfigValueNode(child[j]);
                                if (nestedArrayResult) {
                                    return nestedArrayResult;
                                }
                            }
                        } else if (child && typeof child === 'object') {
                            var nestedObjectResult = findScriptConfigValueNode(child);
                            if (nestedObjectResult) {
                                return nestedObjectResult;
                            }
                        }
                    }
                ...
                var configNode = findScriptConfigValueNode(ast);
                if (configNode) {
                    result = astToJavaScript(configNode);
                }

                if (!result) {
                    result = tryExtractConfigFromAssignedObjectLiteral(scriptSource);
                }
